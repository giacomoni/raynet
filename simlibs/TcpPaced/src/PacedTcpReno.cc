...
#include "PacedTcpReno.h"

namespace learning {

Register_Class(PacedTcpReno);

PacedTcpReno::PacedTcpReno()
{
}
...
void PacedTcpReno::processRexmitTimer(TcpEventCode &event)
{
    ...
    pacedConn->changeIntersendingTime(state->srtt.dbl() / ((double) state->snd_cwnd / (double) state->snd_mss));
    ...
    if (state->dupacks >= DUPTHRESH) {    // DUPTHRESH = 3
        ...
        if (state && state->ect && state->gotEce) {
            ...
            if (simTime() - state->eceReactionTime > state->srtt) {
                ...
                state->sndCwr = true;
                performSsCa = false;
                EV_INFO << "ssthresh = cwnd/2: received ECN-Echo ACK... new ssthresh = " << state->ssthresh << "\n";
...
void PacedTcpReno::receivedDuplicateAck()
{
