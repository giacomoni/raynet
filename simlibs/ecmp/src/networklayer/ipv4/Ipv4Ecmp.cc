...
void Ipv4Ecmp::initialize(int stage)
{
...
void Ipv4Ecmp::handleRegisterService(const Protocol& protocol, cGate *out, ServicePrimitive servicePrimitive)
{
    Enter_Method("handleRegisterService");
}

void Ipv4Ecmp::handleRegisterProtocol(const Protocol& protocol, cGate *in, ServicePrimitive servicePrimitive)
{
...
void Ipv4Ecmp::refreshDisplay() const
{
...
void Ipv4Ecmp::handleRequest(Request *request)
{
...
void Ipv4Ecmp::handleMessageWhenUp(cMessage *msg)
{
...
bool Ipv4Ecmp::verifyCrc(const Ptr<const Ipv4Header>& ipv4Header)
{
...
Packet *Ipv4Ecmp::prepareForForwarding(Packet *packet) const
{
...
void Ipv4Ecmp::preroutingFinish(Packet *packet)
{
...
void Ipv4Ecmp::handlePacketFromHL(Packet *packet)
{
...
void Ipv4Ecmp::datagramLocalOut(Packet *packet)
{
 ...
 */
const InterfaceEntry *Ipv4Ecmp::determineOutgoingInterfaceForMulticastDatagram(const Ptr<const Ipv4Header>& ipv4Header, const InterfaceEntry *multicastIFOption)
{
...
void Ipv4Ecmp::routeUnicastPacket(Packet *packet)
{
...
void Ipv4Ecmp::routeUnicastPacketFinish(Packet *packet)
{
...
void Ipv4Ecmp::routeLocalBroadcastPacket(Packet *packet)
{
...
const InterfaceEntry *Ipv4Ecmp::getShortestPathInterfaceToSource(const Ptr<const Ipv4Header>& ipv4Header) const
{
    return rt->getInterfaceForDestAddr(ipv4Header->getSrcAddress());
}

void Ipv4Ecmp::forwardMulticastPacket(Packet *packet)
{
    const InterfaceEntry *fromIE = ift->getInterfaceById(packet->getTag<InterfaceInd>()->getInterfaceId());
    const auto& ipv4Header = packet->peekAtFront<Ipv4Header>();
...
void Ipv4Ecmp::reassembleAndDeliver(Packet *packet)
{
    EV_INFO << "Delivering " << packet << " locally.\n";
...
void Ipv4Ecmp::reassembleAndDeliverFinish(Packet *packet)
{
...
void Ipv4Ecmp::decapsulate(Packet *packet)
{
...
void Ipv4Ecmp::fragmentPostRouting(Packet *packet)
{
...
void Ipv4Ecmp::setComputedCrc(Ptr<Ipv4Header>& ipv4Header)
{
...
void Ipv4Ecmp::insertCrc(const Ptr<Ipv4Header>& ipv4Header)
{
...
void Ipv4Ecmp::fragmentAndSend(Packet *packet)
{
    const InterfaceEntry *destIE = ift->getInterfaceById(packet->getTag<InterfaceReq>()->getInterfaceId());
    Ipv4Address nextHopAddr = getNextHop(packet);
...
void Ipv4Ecmp::encapsulate(Packet *transportPacket)
{
...
void Ipv4Ecmp::sendDatagramToOutput(Packet *packet)
{
...
void Ipv4Ecmp::arpResolutionCompleted(IArp::Notification *entry)
{
...
void Ipv4Ecmp::arpResolutionTimedOut(IArp::Notification *entry)
{
    if (entry->l3Address.getType() != L3Address::IPv4)
        return;
    auto it = pendingPackets.find(entry->l3Address.toIpv4());
...
MacAddress Ipv4Ecmp::resolveNextHopMacAddress(cPacket *packet, Ipv4Address nextHopAddr, const InterfaceEntry *destIE)
{
...
void Ipv4Ecmp::sendPacketToNIC(Packet *packet)
{
...
void Ipv4Ecmp::registerHook(int priority, INetfilter::IHook *hook)
{
    Enter_Method("registerHook()");
    NetfilterBase::registerHook(priority, hook);
}

void Ipv4Ecmp::unregisterHook(INetfilter::IHook *hook)
{
    Enter_Method("unregisterHook()");
    NetfilterBase::unregisterHook(hook);
}

void Ipv4Ecmp::dropQueuedDatagram(const Packet *packet)
{
...
void Ipv4Ecmp::reinjectQueuedDatagram(const Packet *packet)
{
...
INetfilter::IHook::Result Ipv4Ecmp::datagramPreRoutingHook(Packet *packet)
{
    for (auto & elem : hooks) {
...
INetfilter::IHook::Result Ipv4Ecmp::datagramForwardHook(Packet *packet)
{
    for (auto & elem : hooks) {
...
INetfilter::IHook::Result Ipv4Ecmp::datagramPostRoutingHook(Packet *packet)
{
    for (auto & elem : hooks) {
...
void Ipv4Ecmp::handleStartOperation(LifecycleOperation *operation)
{
    start();
}

void Ipv4Ecmp::handleStopOperation(LifecycleOperation *operation)
{
    // TODO: stop should send and wait pending packets
    stop();
}

void Ipv4Ecmp::handleCrashOperation(LifecycleOperation *operation)
{
    stop();
}
...
INetfilter::IHook::Result Ipv4Ecmp::datagramLocalInHook(Packet *packet)
{
    for (auto & elem : hooks) {
...
INetfilter::IHook::Result Ipv4Ecmp::datagramLocalOutHook(Packet *packet)
{
    for (auto & elem : hooks) {
...
void Ipv4Ecmp::receiveSignal(cComponent *source, simsignal_t signalID, cObject *obj, cObject *details)
{
    Enter_Method_Silent();
...
void Ipv4Ecmp::sendIcmpError(Packet *origPacket, int inputInterfaceId, IcmpType type, IcmpCode code)
{
